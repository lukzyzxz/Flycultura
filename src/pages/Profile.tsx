...
import { User, Heart, ShoppingBag, Settings, LogOut, MapPin, Calendar } from "lucide-react";
          ...
          <div className="space-y-4">
            {orders.length === 0 ? (
              <div className="text-center py-16">
                ...
                <Link to="/packages">
                  <Button className="mt-4">{t("cart.browsePackages")}</Button>
                </Link>
              </div>
            ) : (
              orders.map((order, i) => (
                <motion.div
                  ...
                  className="rounded-xl bg-card card-shadow p-5"
                >
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      <p className="text-sm text-muted-foreground">
                        {new Date(order.created_at).toLocaleDateString(locale === "pt" ? "pt-BR" : "en-US")}
                      </p>
                      <p className="text-xs text-muted-foreground font-mono">#{order.id.slice(0, 8)}</p>
                    </div>
                    <div className="flex items-center gap-3">
                      <Badge variant={order.status === "completed" ? "default" : "secondary"}>
                        {order.status === "completed" ? (locale === "pt" ? "Concluído" : "Completed") :
                         order.status === "pending" ? (locale === "pt" ? "Pendente" : "Pending") : order.status}
                      </Badge>
          ...
          <div className="max-w-md space-y-6">
            <div className="rounded-xl bg-card card-shadow p-6 space-y-4">
              <h3 className="font-display font-bold text-card-foreground">
                {locale === "pt" ? "Informações da Conta" : "Account Info"}
              </h3>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-muted-foreground">{locale === "pt" ? "Nome" : "Name"}</span>
                ...
                <div className="flex justify-between">
                  <span className="text-muted-foreground">{locale === "pt" ? "Membro desde" : "Member since"}</span>
                  <span className="text-foreground">
                    {new Date(user.created_at).toLocaleDateString(locale === "pt" ? "pt-BR" : "en-US")}
                  </span>
                </div>
              </div>
            </div>
