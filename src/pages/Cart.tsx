        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-4">
            {items.map((item, i) => (
              ...
              >
                <Link to={`/packages/${item.package.id}`} className="shrink-0">
                  <img
                ...
                <div className="flex-1 min-w-0">
                  <Link to={`/packages/${item.package.id}`}>
                    <h3 className="font-display font-bold text-card-foreground truncate hover:text-primary transition-colors">
                      {item.package.event}
                    </h3>
                  </Link>
                  <p className="text-sm text-muted-foreground">{item.package.location} — {item.package.date}</p>
                ...
                <div className="text-right shrink-0">
                  ...
                  {item.quantity > 1 && (
                    <p className="text-xs text-muted-foreground">
                      R$ {item.package.price.toLocaleString("pt-BR")} {t("events.perPerson")}
                    </p>
                  )}
                </div>
              </motion.div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 rounded-xl bg-card card-shadow p-6 space-y-4">
              ...
              <div className="space-y-2 text-sm">
                {items.map((item) => (
                  <div key={item.package.id} className="flex justify-between text-muted-foreground">
                    <span className="truncate mr-2">{item.package.event} x{item.quantity}</span>
                    <span>R$ {(item.package.price * item.quantity).toLocaleString("pt-BR")}</span>
                  </div>
                ))}
              </div>
