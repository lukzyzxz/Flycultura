      ...
      <div className="container relative z-10">
        ...
        >
          <h1 className="font-display text-4xl md:text-6xl font-extrabold text-primary-foreground mb-4 leading-tight">
            {t("hero.title1")}<br />
            <span className="opacity-80">{t("hero.title2")}</span>
          </h1>
          ...
          <div className="bg-card rounded-xl p-4 md:p-6 card-shadow">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              <div className="relative">
                ...
                <Input placeholder={t("hero.from")} value={from} onChange={(e) => setFrom(e.target.value)} className="pl-9" />
              </div>
              <div className="relative">
                <MapPin className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                <Input placeholder={t("hero.to")} value={to} onChange={(e) => setTo(e.target.value)} className="pl-9" />
              </div>
              <div className="relative">
